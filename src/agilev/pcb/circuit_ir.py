...
class Component:
    ...
    def to_dict(self) -> dict[str, Any]:
        """Convert to dictionary."""
        d = {"id": self.id, "type": self.type, "pins": [p.to_dict() for p in self.pins]}
...
class Interface:
    ...
    def to_dict(self) -> dict[str, Any]:
        """Convert to dictionary."""
        d = {"name": self.name, "type": self.type, "signals": self.signals}
        if self.voltage is not None:
            d["voltage"] = self.voltage
...
class CircuitIR:
    ...
    def from_dict(cls, data: dict[str, Any]) -> "CircuitIR":
        ...
        for net_data in data.get("nets", []):
            connections = [
                (conn["component"], conn["pin"]) for conn in net_data.get("connections", [])
            ]
            net = Net(
                name=net_data["name"],
        ...
        for pd_data in data.get("power_domains", []):
            pd = PowerDomain(
                name=pd_data["name"],
                voltage_nominal=pd_data.get(
                    "voltage_nominal", pd_data.get("voltage", 3.3)
    ...
    def validate_connections(self) -> list[str]:
        ...
        for net in self.nets:
            for comp_id, pin_number in net.connections:
                ...
                if comp is None:
                    ...
                    if not pin_found:
                        errors.append(
                            f"Net '{net.name}' references non-existent pin '{pin_number}' on component '{comp_id}'"
                        )
        ...
        for pd in self.power_domains:
            for net_name in pd.nets:
                if not self.get_net(net_name):
                    errors.append(
                        f"Power domain '{pd.name}' references non-existent net '{net_name}'"
                    )
        ...
        for interface in self.interfaces:
            if interface.master and not self.get_component(interface.master):
                errors.append(
                    f"Interface '{interface.name}' references non-existent master '{interface.master}'"
                )

            for slave_id in interface.slaves:
                if not self.get_component(slave_id):
                    errors.append(
                        f"Interface '{interface.name}' references non-existent slave '{slave_id}'"
                    )

            for signal_name, net_name in interface.signals.items():
                if not self.get_net(net_name):
                    errors.append(
                        f"Interface '{interface.name}' signal '{signal_name}' references non-existent net '{net_name}'"
                    )
...
def create_capacitor(
    ref: str, value: str, package: str = "0603", voltage_rating: float = None
) -> Component:
