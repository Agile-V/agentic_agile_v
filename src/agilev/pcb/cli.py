...
def cmd_pcb_validate(args: argparse.Namespace) -> int:
    ...
    # Load circuit IR
    circuit_path = Path(f".agentic-agile-v/tasks/{task_id}/candidates/{candidate_id}/circuit_ir.json")
...
def cmd_pcb_components(args: argparse.Namespace) -> int:
    ...
    if args.list:
        components = index.find_approved() if args.approved_only else list(index.components.values())
        ...
        for comp in components:
            ...
            print(f"{comp.part_number:<20} {comp.manufacturer:<20} {comp.description[:40]:<40} {approved:<10}")
...
def build_pcb_parser(subparsers) -> None:
    ...
    comp_parser.add_argument("--approved-only", action="store_true", help="Show only approved components")
