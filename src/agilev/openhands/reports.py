...
class ReportGenerator:
    ...
    def generate_handoff(self, task_id: str) -> HandoffReport:
        ...
        recommendations = self._generate_recommendations(task_brief, evidence, tests, checks, scope)

        # Identify residual risks
        residual_risks = self._identify_residual_risks(task_brief, evidence, tests, checks)
...
"""
        ...
        if report.check_failures:
            ...
            for check in report.check_failures:
                md += (
                    f"- **{check.get('type', 'unknown')}**: {check.get('message', 'No message')}\n"
                )
            md += "\n"
...
"""
    ...
    def _generate_recommendations(
        ...
        if failed_checks:
            recommendations.append(f"Address {len(failed_checks)} quality check failure(s)")

        # Dependency changes
        if scope.get("dependency_changes"):
            recommendations.append("Review dependency changes for security vulnerabilities")

        # Public API changes
        if scope.get("public_api_changed"):
            recommendations.append("Update API documentation and notify downstream consumers")

        # Scope violations
        if scope.get("violations"):
            recommendations.append("Justify scope violations or remove out-of-scope changes")

        # High risk
        if task_brief.get("risk_level") in ["L3", "L4"]:
            recommendations.append("Obtain security team approval before deployment")
            recommendations.append("Plan for gradual rollout with monitoring")
