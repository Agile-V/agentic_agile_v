...
class SessionConfig:
    ...
    verify_previous_session: str | None = None  # Session ID to verify
...
class OpenHandsSessionManager:
    ...
    def start_session(self, session_id: str, prompt: str) -> None:
        ...
        with open(log_file, "w") as log:
            try:
                ...
                # Update metadata
                metadata.state = (
                    SessionState.COMPLETED if result.returncode == 0 else SessionState.FAILED
                )
        ...
        self.ledger.append_event(
            ...
            details={
                ...
                "duration_seconds": (metadata.completed_at - metadata.started_at).total_seconds()
    ...
    def list_sessions(self, task_id: str | None = None) -> list[SessionMetadata]:
        """
        List sessions.
    ...
    def _build_openhands_command(
        self, config: dict[str, Any], prompt_file: Path, session_dir: Path
    ) -> list[str]:
        """
        ...
        if docker_check.stdout.strip():
            ...
            return [
                ...
                "-e",
                f"AGILEV_TASK_ID={config['task_id']}",
                "-e",
                f"AGILEV_AGENT_ROLE={config['role']}",
...
class BuilderVerifierWorkflow:
    ...
    def run(self, task_id: str, implementation_prompt: str) -> dict[str, Any]:
        ...
        """
        results = {"task_id": task_id, "cycles": [], "final_state": None, "approved": False}
    ...
    def _build_builder_prompt(
        ...
        cycle: int,
        previous_verifier_session: str | None,
    ) -> str:
