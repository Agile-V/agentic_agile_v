...
def cmd_init(args: argparse.Namespace) -> int:
    ...
    print(f"  - .agentic-agile-v/state/events.jsonl")
    print(f"  - .agentic-agile-v/state/tasks.json")
    print(f"  - .agentic-agile-v/state/locks.json")
    
    print(f"\n📝 Next steps:")
    print(f"  1. Run: agilev new --title 'Your task name' --risk L1")
    print(f"  2. Create your task brief in the generated directory")
    print(f"  3. Run: agilev validate to check your work")
...
def cmd_new(args: argparse.Namespace) -> int:
    ...
    brief = {
        ...
        "scope": [
            "TODO: What is included in this task"
        ],
        "non_goals": [
        ...
        "constraints": [
            "TODO: Technical or business constraints"
        ],
        "acceptance_criteria": [
            "TODO: Criteria for acceptance"
        ],
        "risk_level": args.risk,
        "affected_components": [
            "TODO: Components that will be changed"
        ],
        "required_evidence": [],
    ...
    (task_dir / "plan.md").write_text(f"# Implementation Plan: {task_id}\n\nTODO: Create implementation plan\n")
...
def cmd_brief(args: argparse.Namespace) -> int:
    ...
    required_fields = [
        "task_id", "title", "problem_statement", "intended_outcome",
        "scope", "non_goals", "requirements", "constraints",
        "acceptance_criteria", "risk_level", "affected_components",
        "required_evidence", "human_approval_required"
    ]
    
    missing_fields = [field for field in required_fields if field not in brief or not brief[field]]
    
    if missing_fields:
        print(f"❌ Task brief validation FAILED")
...
def cmd_classify(args: argparse.Namespace) -> int:
    ...
    if policy_path.exists():
        ...
        print(f"⚠️  Risk policy not found, using defaults")
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if event_logger.log_path.exists():
        with open(event_logger.log_path, 'r') as f:
...
def cmd_lock(args: argparse.Namespace) -> int:
    ...
    if success:
        ...
        print(f"❌ Lock conflict: One or more files are already locked")
...
def cmd_openhands_init(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        other = {k: v for k, v in created_files.items() 
        ...
        print(f"\n📝 Next steps:")
        print(f"  1. Run: agilev openhands doctor")
        print(f"  2. Create a task: agilev new --title 'Task name' --risk L1")
        print(f"  3. Run OpenHands with Agile-V skills and hooks active")
...
def cmd_openhands_validate(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if session_file.exists():
            print(f"  ✅ OpenHands session metadata found")
        else:
            print(f"  ⚠️  OpenHands session metadata not found")
        ...
        if tool_log.exists():
            ...
            print(f"  ⚠️  OpenHands tool log not found")
        
        # Scope validation if requested
        if args.scope:
            print(f"\n🎯 Validating scope...")
            # TODO: Implement scope validation
            print(f"  ⚠️  Scope validation not yet implemented (MVP)")
...
def cmd_openhands_run(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if args.builder_verifier:
            ...
            # Display results
            print(f"\n{'='*60}")
            print(f"Builder/Verifier Workflow Complete")
            print(f"{'='*60}")
...
def cmd_openhands_sessions(args: argparse.Namespace) -> int:
    ...
    for session in sessions:
        ...
        print(f"{session.session_id:<40} {session.task_id:<12} {session.role.value:<10} {session.state.value:<12} {created:<20}")
...
def cmd_openhands_events(args: argparse.Namespace) -> int:
    ...
    for event in events[-args.limit:]:  # Show last N events
        ...
        print(f"{event.event_id:<15} {event.event_type.value:<25} {event.task_id or 'N/A':<12} {event.actor:<20} {timestamp:<20}")
        if args.verbose:
...
def cmd_openhands_verify_chain(args: argparse.Namespace) -> int:
    ...
    if is_valid:
        ...
        if summary['first_event']:
            print(f"  First event: {summary['first_event']['id']} ({summary['first_event']['timestamp']})")
        if summary['last_event']:
            print(f"  Last event: {summary['last_event']['id']} ({summary['last_event']['timestamp']})")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    new_parser.add_argument("--risk", choices=["L0", "L1", "L2", "L3", "L4"], default="L1",
    ...
    # openhands init
    oh_init_parser = openhands_subparsers.add_parser("init", help="Initialize OpenHands integration")
    ...
    # openhands scaffold
    oh_scaffold_parser = openhands_subparsers.add_parser("scaffold", help="Regenerate integration files")
    ...
    # openhands validate
    oh_validate_parser = openhands_subparsers.add_parser("validate", help="Validate OpenHands session")
    ...
    oh_evidence_parser = openhands_subparsers.add_parser("evidence", help="Collect evidence from OpenHands session")
    oh_evidence_subparsers = oh_evidence_parser.add_subparsers(dest="evidence_command", required=True)
    
    oh_evidence_collect_parser = oh_evidence_subparsers.add_parser("collect", help="Collect evidence")
    ...
    oh_run_parser.add_argument("--builder-verifier", action="store_true", 
                               help="Use builder/verifier workflow")
    oh_run_parser.add_argument("--max-iterations", type=int, default=50,
                               help="Maximum iterations (default: 50)")
    oh_run_parser.add_argument("--timeout", type=int, default=3600,
                               help="Timeout in seconds (default: 3600)")
    oh_run_parser.add_argument("--model", default="gpt-4",
    ...
    oh_gh_parser = openhands_subparsers.add_parser("github-actions", help="Generate GitHub Actions workflows")
    ...
    oh_events_parser.add_argument("--limit", type=int, default=50, help="Max events to show (default: 50)")
    oh_events_parser.add_argument("--verbose", "-v", action="store_true", help="Show full event details")
    oh_events_parser.set_defaults(func=cmd_openhands_events)
    
    # openhands verify-chain
    oh_verify_parser = openhands_subparsers.add_parser("verify-chain", help="Verify event chain integrity")
